...
## Application Launch Preview Core Implementation v1

Application Launch Subphase 1 is implemented as an exact desktop-entry preview service only. It accepts a user-supplied `.desktop` ID or path inside `/usr/share/applications` or `~/.local/share/applications`; it never enumerates applications. The resolver validates `Type=Application`, `Name`, and a shell-free `Exec`, rejects file/URL placeholders, resolves the executable without running it, and returns canonical entry path, argv, executable path, and entry SHA-256.

`POST /api/v1/device-agent/launch-preview` combines this preview with the existing Launch permission policy and reports `execution_enabled=false`. Fixture-based unit/API tests verify exact resolution, placeholder rejection, out-of-scope rejection, and non-execution. Full synthetic verification completed with 66 passing tests and a successful frontend build; no actual application was launched.

## Application Launch Execution Core Implementation v1
